...
function shouldValidateDocument(document: vscode.TextDocument): boolean {
  ...
  const hasDirective = /(^|\n)\s*:::(material|info|task|subtask|lines|grid|geometry|dots|space|table|numberline|mc|cloze|matching|wordsearch|solution|columns|nextcol|endcolumns|help|hilfe|qrcode|pagebreak|framebreak|slidechromeoff|sectionmark|vspacer)\b/m.test(text);
