...
- Der Schreibbereich war auch dann bearbeitbar und per Ansichtswechsel (Nur Schreibbereich/Beides) fokussierbar, wenn gar kein Dokument geladen war (z. B. direkt nach dem Start oder nach dem Schließen des letzten Tabs) -- dadurch landete der Tastaturfokus im leeren Editor und die globalen Kurzbefehle `o` (Datei öffnen) und `z` (zuletzt geöffnete Datei) reagierten nicht mehr, sobald zuvor in die Editor- oder Splitscreen-Ansicht gewechselt wurde. Der Schreibbereich ist jetzt sichtbar deaktiviert und nimmt den Fokus nicht mehr an, solange kein Dokument aktiv ist; globale Kurzbefehle funktionieren dadurch zuverlässig auch direkt nach einem Ansichtswechsel. Zusätzlich: Ein Tab-Wechsel zu einem Dokument, dessen Datei zwischenzeitlich nicht mehr gelesen werden kann (gelöscht/keine Berechtigung), zeigte bisher weiterhin den Inhalt des vorherigen Tabs an -- der Schreibbereich wird in diesem Fall jetzt korrekt geleert und deaktiviert statt den falschen Inhalt unter dem neuen Tab stehen zu lassen.
  **Nachbesserung an derselben Stelle** (kurz nach der obigen Änderung entdeckt, hier transparent mitdokumentiert statt stillschweigend nachgebessert): Ein Klick in den deaktivierten Schreibbereich fokussierte ihn trotzdem (Tk fokussiert ein `Text`-Widget per Mausklick unabhängig vom deaktivierten Zustand) -- `o`/`z` blieben dadurch nach jedem Klick erneut blockiert, behoben durch einen zusätzlichen Klick-Schutz. Beim Schließen des letzten Tabs blieben zudem die Diagnostik-Liste und die Struktur-Übersicht mit den Einträgen des vorherigen Dokuments stehen -- werden jetzt korrekt geleert. **Kritisch:** Das zuletzt-geöffnet-Kürzel `z` (sowie jeder andere Ladevorgang direkt nach Start/Tab-Schließen) öffnete die Datei zwar in der Vorschau, ließ den Schreibbereich aber leer, obwohl er als "geladen" galt -- ein anschließendes Tippen und Kompilieren hätte die Datei praktisch geleert und gespeichert. Behoben; der Ladevorgang committet neue Dokumentinhalte und -Baselines jetzt nur noch gemeinsam nach vollständigem Erfolg.
- Die Einstellung "Startverhalten" für die Editor-Ansicht (z. B. "Beides"/Splitscreen) wurde beim App-Start zwar korrekt geladen, direkt danach aber von einer Altlast-Zeile wieder auf "Nur Vorschau" zurückgesetzt -- Blattwerk startete dadurch immer in der Vorschau-Ansicht, unabhängig von der gewählten Präferenz. Behoben.
...
- Die "Schrift"-Auswahl zeigte nach einem Wechsel weiterhin die zuvor gewählte Schriftart an (die neue Auswahl griff aber tatsächlich, sichtbar erst nach erneutem Öffnen des Menüs). Behoben.
- Die Vorschau-Werkzeugleiste zeigt bei einem Kurzentwurf-Tab jetzt nur noch tatsächlich wirksame Steuerelemente (Vorschau-Navigation/Zoom/Aktualisieren) -- Seitenformat, Black-Screen, Phasen, Kontrast/Farbprofil/Schrift/Größe und der Aufgabe/Lösung-Umschalter hatten dort bisher keinerlei Effekt (der Aufgabe/Lösung-Umschalter sprang bei jedem Aktualisieren sogar sichtbar wirkungslos auf "Aufgabe" zurück), blieben aber sichtbar bedienbar. "Lernhilfen" ist für Kurzentwurf jetzt bewusst deaktiviert statt nur zufällig, weil kein `:::`-Block vorhanden war.
- Black-Screen (Vorher/Nachher/Beides) ist jetzt wie "Phasen" nur noch im Präsentationsmodus aktiv -- im normalen Aufgabenblatt-Modus war es bisher bedienbar, obwohl es dort keine Wirkung hat.
- Formeln (`$...$`/`$$...$$`) wirkten beim Export immer optisch "fett"/schwer im Vergleich zum umgebenden Fließtext, obwohl nirgends `font-weight: bold` gesetzt war -- Ursache war MathJax' bisherige Standard-Formelschrift (Version 3), die neben schlanken UI-Schriftarten wie Segoe UI von Haus aus schwerer wirkt. MathJax auf Version 4 aktualisiert (`blatt_kern_layout_render.py`, CDN-Skript in `render_html`/`_render_presentation_html`), deren neue Standardschrift laut MathJax-Projekt gezielt für bessere Passung mit schlanken Windows-Schriftarten entwickelt wurde.
- `:::solution label=false`-Boxen hatten über dem Inhalt mehr Abstand als darunter, weil der Standard-Absatzabstand des ersten Kindelements (anders als beim letzten) nicht entfernt wurde -- bei `label=true` fiel das nicht auf, da dort die Beschriftung das erste Kindelement war. Der Innenabstand ist jetzt oben und unten symmetrisch (`worksheet.css`, `.solution > *:first-child`).

### Added

- **Dateien per „Öffnen mit“ in Blattwerk öffnen:** `.md`-Dateien lassen sich per Rechtsklick → Öffnen mit → Blattwerk öffnen (einmalige Einrichtung über `register-blattwerk-file-association.ps1`, Schritt-für-Schritt-Anleitung in `docs/nutzer/OEFFNEN_MIT_EINRICHTEN.md`). Läuft Blattwerk bereits, öffnet sich die Datei als neuer Tab im bestehenden Fenster statt in einem zweiten – auch wenn mehrere Dateien gleichzeitig geöffnet werden. Ein weiterer Start ohne Datei holt das laufende Fenster nach vorn, statt ein neues zu öffnen. `start-blattwerk.bat` und `blattwerk.py` akzeptieren außerdem einen Dateipfad als Argument; eine so übergebene Datei hat Vorrang vor „Beim Start letzte Datei laden“.
- Neue **Worterklärung** als sprachliche Hilfe: `??Begriff|Erklärung??` lässt den Begriff normal im Text stehen (mit gepunkteter Unterstreichung) und setzt die Erklärung klein an den rechten Rand des Arbeitsblatts, ungefähr auf Höhe des Absatzes. Sobald ein Arbeitsblatt mindestens eine Worterklärung enthält, wird rechts eine Randspalte reserviert (der Text wird entsprechend schmaler), sonst bleibt die volle Breite. Begriff und Erklärung sind reiner Text (`*x*` bleibt `*x*`); `\|` und `\??` schreiben ein wörtliches `|` bzw. `??`. Fehlendes oder doppeltes `|` sowie leerer Begriff/leere Erklärung sind ein Fehler (`IM002`) und blockieren den Export. Passt eine Erklärung nicht mehr auf die Seite ihres Absatzes, wird sie nicht abgeschnitten, sondern läuft im Rand der Folgeseite weiter (endet das Dokument dort, entsteht dafür eine zusätzliche Seite); der Export warnt dann (`IM003`). Nicht unterstützt: Worterklärungen in Tabellenzellen und in allen `:::columns`-Spalten außer der letzten (dort erscheint die Erklärung nicht, ebenfalls mit `IM003`-Warnung); in Präsentationen und im Kurzentwurf wird nur der Begriff angezeigt.
- Im `:::`-Autocomplete-Popup (Operatoren, Frontmatter-Schlüssel, Blocktypen) markiert jetzt auch reines Drüberfahren mit der Maus (ohne Klick) den Eintrag und aktualisiert die Erklärungsspalte daneben -- bisher reagierte die Liste nur auf Klick oder Pfeiltasten, anders als das gleichartige Ctrl+B-Einfügemenü, das dieses Hover-Verhalten bereits hatte.
- Der experimentelle editierbare PPTX-Export (siehe Eintrag weiter unten) exportiert `:::table`-Blöcke jetzt als echte, zellbasierte PowerPoint-Tabelle statt als Bild -- Zelltext, fett formatierte Zellen (z. B. Kopfzeilen), Ausrichtung, Hintergrundfarbe und zusammengeführte Zellen (`colspan`/`rowspan`) bleiben in PowerPoint direkt bearbeitbar. Rahmenfarben/-breiten, Zellabstände und die vertikale Ausrichtung werden nicht übertragen; gemischte Textformatierung (`**fett**`) innerhalb einer einzelnen Zelle bleibt unformatierter Text (dokumentierte v1-Grenze).
- Der experimentelle editierbare PPTX-Export (siehe Eintrag weiter unten) erhält jetzt gemischte Inline-Formatierung innerhalb eines Absatzes -- `**fett** normaler *kursiv* Text` wird als eine Textbox mit mehreren echten PowerPoint-Runs exportiert, jeder Run mit seiner eigenen Fett-/Kursiv-Formatierung, statt bisher für den ganzen Absatz nur eine einzige Formatierung (die des ersten Zeichens) zu übernehmen. Kursiv wird damit erstmals überhaupt erkannt. Feinere CSS-Schriftgewichte (z. B. 500/600) bleiben v1 weiterhin binär auf fett/nicht-fett reduziert.
- Der experimentelle editierbare PPTX-Export (siehe Eintrag weiter unten) verwendet für echte `<img>`-Bildverweise jetzt die Original-Bilddatei statt eines Browser-Screenshots -- bessere Bildqualität, kleinere Dateigröße. Formeln/Diagramme/Chrome bleiben unverändert Screenshots.
...
- Der Beschreibungstext des Frontmatter-Felds `Stufe` (u. a. sichtbar in der neuen Erklärungsspalte) war veraltet und behauptete, `Stufe` habe keinerlei Wirkung -- tatsächlich schränkt es ein, welche `!!Operator!!`-Bezeichnungen im Dokument vorgeschlagen/gültig sind. Text korrigiert.
- Autocomplete-Popups (Operatoren, Frontmatter-Schlüssel wie `mode`/`Stufe`, Blocktypen nach `:::`) zeigen jetzt eine zusätzliche Erklärungs-Spalte neben der Vorschlagsliste: eine kurze Beschreibung des gerade markierten Eintrags, bei Frontmatter-Schlüsseln zusätzlich ein Standard- bzw. möglicher Wert. Die Spalte aktualisiert sich mit Pfeiltasten und Mausklick und bleibt komplett ausgeblendet, wenn für den markierten Eintrag keine Erklärung vorliegt (aktuell: Block-Optionen wie `rows=` und deren Werte noch ohne Erklärung -- geplante Folge-Erweiterung).
- Beim Tippen innerhalb eines offenen `!!...!!`-Operator-Markers schlägt der Editor jetzt passende Operator-Bezeichnungen vor (z. B. `!!Best` → `Bestimmen`) -- abgestimmt auf das im Frontmatter gesetzte `Fach` und, falls vorhanden, `Stufe`. Vorgeschlagen wird immer nur die offizielle Bezeichnung, keine Konjugationsformen.
- Warnungen in der Diagnostik-Liste unter dem Editor lassen sich jetzt einzeln über eine Checkbox als gelesen abhaken (alternativ per Rechtsklick, dort auch "Alle in diesem Dokument wieder anzeigen"). Abgehakte Warnungen bleiben sichtbar (nur grau dargestellt), lösen aber beim nächsten Kompilieren/Export kein Warnungs-Popup mehr aus -- außer die Warnung verschwindet zwischenzeitlich (z. B. weil die Ursache behoben wurde) und tritt später erneut auf, dann gilt sie wieder als neu. Der Abhak-Zustand ist an "Zuletzt geöffnet" gekoppelt: fällt ein Dokument aus dieser Liste, kann sein Abhak-Zustand mit verschwinden. Betrifft nur Warnungen, keine Fehler.
- Neue Syntax `!!Operator!!` markiert einen Aufgaben-Operator (z. B. `!!Bestimmen Sie!! die Nullstellen.`) -- sieht optisch identisch zu `**fett**` aus, aber am Ende des Arbeitsblatts erscheint automatisch eine Legende mit den Definitionen aller tatsächlich verwendeten Operatoren (nicht in der Lösung, nicht im Kurzentwurf). Für Mathematik liegt bereits die vollständige, offizielle Operatorenliste bei (Niedersächsisches Kultusministerium, Zentralabitur ab 2021); weitere Fächer lassen sich als eigene Datei in `data/operatoren/` ergänzen. Das Frontmatter-Feld `Stufe` (`5`-`13`, `E`, `Q1`, `Q2`, `Sek1`, `Sek2`) kann Operatoren zusätzlich auf bestimmte Klassenstufen einschränken und bekommt jetzt auch Editor-Autocomplete für seine Werte (wie schon `mode`/`document_type`).
- `:::crossword` bekommt vier neue Optionen, um Hinweis-Nummern und Codewort-Positionen unabhängig voneinander im Stil zu unterscheiden: `numbering=numeric|letters|symbols` für die bestehenden Hinweis-Nummern, und neu `code_numbering=numeric|letters|symbols` (Standard `none`, also unverändert wie bisher) für eine kleine Positionsmarkierung an jeder Codewort-Zelle (die wievielte Stelle im Lösungswort). Bei `symbols` wählt `symbol_set=`/`code_symbol_set=` (`fruits`/`animals`/`plants`) ein kindgerechtes Emoji-Thema. So kann z. B. die Hinweis-Nummerierung durchgehend Zahlen bleiben, während das Codewort durchgehend Buchstaben oder Tier-Symbole bekommt.
- Text im Editor markieren und eine Taste drücken formatiert die Auswahl direkt: `*`/`_` eskalieren stufenweise (kursiv -> fett -> fett+kursiv bzw. kursiv -> unterstrichen), `=` fügt `==Hervorhebung==` ein (Farbe folgt jetzt dem aktiven Farbprofil, nicht mehr nur in Kurzentwurf), `~` schaltet zwischen Tiefstellung und Durchstreichen, `^` stellt hoch, `` ` `` wechselt zwischen Inline-Code (einzeilige Auswahl) und Codeblock (mehrzeilige Auswahl), `|` fügt einen Spoiler ein (nur beim Markieren im PDF lesbar), `$` umschließt die Auswahl mit einer Formel (`$...$`). Zusätzlich neu: `%%Kommentar%%`-Syntax für Autor:innen-Notizen, die in keiner Ausgabe erscheinen. Alle Formatierungen funktionieren jetzt einheitlich in Arbeitsblatt/Präsentation, Antwortfeldern und Kurzentwurf-Zellentext, ohne bestehende Mathematik (`$...$`), Links oder Tabellen zu beeinträchtigen.
- Schriftgröße kann jetzt pro Tab gemerkt werden, genau wie Schriftart/Kontrast/Farbprofil schon bisher -- togglebar in Einstellungen > Design und Theme ("Schriftgröße pro Tab merken", Standard: an). Dieses Verhalten wurde einmal zuvor bewusst wieder entfernt, weil die Größenauswahl beim Tab-Wechsel implizit umgesprungen war; der Umschalter dient als sofortige Rückfalloption, falls das erneut auftritt.
- Die "Schrift"-Auswahl wurde von 5 auf 20 Schriftfamilien erweitert (u. a. Times New Roman, Georgia, Cambria, Garamond, Century Gothic, Trebuchet MS, Comic Sans MS, Bahnschrift, Corbel, Candara, Rockwell) und zeigt jeden Eintrag jetzt in einer Live-Vorschau in der jeweiligen Schriftart statt als reine Textliste -- deutlich schneller durchprobierbar. Ein Schriftart-Wechsel löst dabei keine erneute Blockberechnung aus (z. B. bleibt ein bereits platziertes Kreuzworträtsel/Wortsuchrätsel unverändert, nur Optik/Export aktualisieren sich).
- Neuer Eintrag "Dokumentation…" im Menü "Extras" öffnet ein Popup mit allen Nutzer-Anleitungen aus `docs/nutzer/` (Nutzerhandbuch, Autoren-Anleitungen, Grammatik, Validator-Referenz, Stilempfehlungen, CSS-Anleitung), gerendert mit Überschriften/Fett/Kursiv/Listen statt roher Markdown-Quelle. Jeder Codeblock bekommt einen eigenen "Kopieren"-Button, der das Beispiel direkt in die Zwischenablage legt -- praktisch, um Blockvorlagen aus den Anleitungen ohne Abtippen ins eigene Dokument zu übernehmen. Das Fenster bleibt nicht-modal geöffnet, sodass parallel weiter im Editor gearbeitet werden kann.
...
- Formeln lassen sich jetzt gezielt fett setzen: `**$formel$**`/`**$$formel$$**` rendert die Formel selbst fett (nicht nur umgebenden Text drumherum), z. B. `**$$x^2 + y^2$$**`. Vorher hatte `**...**` um eine Formel herum keine sichtbare Wirkung, da MathJax Formeln als Vektorgrafik zeichnet, auf die reines `font-weight`/`<strong>` keinen Einfluss hat -- die Formel-Quelle wird jetzt zusätzlich automatisch in `\boldsymbol{...}` eingepackt (`math_span_protection.py::restore_math_spans`/`_wrap_math_span_bold`). Bekannte Grenze: funktioniert nicht für ganze mehrzeilige LaTeX-Umgebungen (z. B. `$$\begin{aligned}...\end{aligned}$$`) in Fett -- dort bitte nur Teile der Formel oder den Text drumherum fett setzen.

### Changed

- **Hauptfenster startet jetzt maximiert** (gilt für alle Programme auf Basis von bw-gui). Ist in Blattwerk „Fenstergeometrie merken“ aktiv und bereits eine Fenstergröße gespeichert, hat diese weiterhin Vorrang.
- **Dokument-Tabs bei vielen offenen Dateien:** Der Schließen-Button (×) rechts der Tab-Leiste bleibt jetzt immer sichtbar – bisher wurde er bei vielen offenen Tabs aus dem Fenster geschoben. Die Tab-Köpfe lassen sich horizontal scrollen (Scrollleiste unter der Leiste, Mausrad über der Leiste); der aktive Tab wird automatisch in den sichtbaren Bereich gescrollt.
- Aufgabensymbole (Operator-, Hinweis- und Arbeitsform-Icon) stehen bei Aufgaben jetzt in einer schmalen Randspalte links neben der Aufgabe statt im Kopf vor dem Label. "Aufgabe N" und der Aufgabentext beginnen dadurch immer an derselben x-Position wie alle anderen Seitenelemente, auch wenn `action=`/`hint=` gesetzt sind. Der ganze Arbeitsblatt-Inhalt rückt dafür um 1,3 cm ein. Der Text neben dem Arbeitsform-Icon ("- Einzelarbeit") entfällt (kein Platz in der Randspalte); die Bezeichnung bleibt als Hover-Text (`title`) des Icons in der Vorschau erhalten. Teilaufgaben, Präsentationen und Spalten (`:::columns`) sind nicht betroffen bzw. zeigen die Symbole weiterhin direkt am Aufgabenkopf.
- Das bisherige `Strg+B`-Einfügemenü (Tastatur-Popup am Cursor) ist entfallen. Blöcke werden jetzt über die neue Menüleisten-Aktion "Einfügen" (`Alt+I`) eingefügt -- die Blocktypen sind darin erstmals in thematische Familien gruppiert (Boxen/Hinweise, Aufgaben, Freies Antwortfeld, Textbasierte Übungen, Grafische/räumliche Übungen, Offen/Reflexion), strukturell einzigartige Blöcke (Spalten-Layout, QR-Code) sowie "Bild" liegen weiterhin direkt auf oberster Ebene. Die Blockerklärung beim Hovern bleibt erhalten, ist aber jetzt robuster: sie erscheint als Untermenü-artiges Flyout ohne Verzögerung und ohne das gelegentliche Hängenbleiben/Fehlpositionieren des alten freischwebenden Tooltips. Das "Einfügen"-Menü (wie jetzt jedes Menü in der Menüleiste) ist außerdem komplett per Tastatur bedienbar: Pfeiltasten wechseln den markierten Eintrag, `→`/Enter öffnet eine Familie bzw. fügt den Block ein, `←`/Escape geht eine Ebene zurück oder schließt das Menü.
- `:::writebox`-Rahmenstile deutlicher unterscheidbar gemacht: `bubble` ist jetzt eine echte Sprechblase mit Schwänzchen, `cloud` hat echte Ausbuchtungen am oberen Rand statt nur weicherer Ecken, `letter` hat einen eleganten Briefpapier-Doppelrahmen statt eines schlichten gestrichelten Rahmens. `frame` bleibt unverändert.
- `:::table`s `headers=`/`row_labels=` heißen jetzt `column_headers=`/`row_headers=`, `header_columns=`/`header_cols=` entfallen komplett -- ohne Rückwärtskompatibilitäts-Alias, bestehende Dokumente mit den alten Namen müssen manuell angepasst werden. Grund: `row_labels=` landete bisher in der ersten Datenspalte und wurde dort von echten `cells:`-Inhalten lautlos verdrängt (das Label erschien einfach nie, wenn Spalte 1 selbst Daten enthielt). `row_headers=` rendert jetzt stattdessen in einer eigenen, zusätzlichen ersten Spalte, die nie mit `cells:`-Daten kollidieren kann; sind `column_headers=` und `row_headers=` gemeinsam gesetzt, bleibt die Eck-Zelle oben links automatisch leer. Beide Optionen unterstützen außerdem jetzt gezielt leere Positionen (z. B. `column_headers="A||C"` lässt die zweite Kopfzelle bewusst leer) statt sie wie bisher stillschweigend zu entfernen.

### Added

- Neue, experimentelle Option beim PPTX-Export von Präsentationen: "Editierbare Text-/Bildelemente" -- statt jede Folie als ein einziges Bild abzulegen, entstehen echte PowerPoint-Textboxen für Fließtext und einzelne, zugeschnittene Bild-Shapes für Grafiken/Formeln/Tabellen. **Kein Versprechen auf vollständige Editierbarkeit:** Tabellen, Formeln, Geometrie-/Raster-/Rätsel-Blöcke und gemischte Inline-Formatierung bleiben (Teil-)Bild, siehe die Unterstützungsmatrix im Modul-Docstring von `app/core/blatt_kern_pptx_export_editable.py`. Benötigt zusätzlich `pip install -r requirements-editable-pptx.txt` (nicht Teil der Standardinstallation) -- ohne das Paket bzw. ohne gefundenen Browser ist die Checkbox deaktiviert bzw. der Export fällt automatisch auf den bisherigen Bild-Export zurück, mit einer sichtbaren Warnung im Export-Dialog, welche Folien/das ganze Dokument davon betroffen sind.
...
- Zwei neue Blocktypen mit eindeutig prüfbarer Lösung:
  ...
  - `:::checkgrid` (kompakte Ankreuz-Tabelle): YAML mit `columns:`/`rows:`; Spaltenköpfe (z. B. "richtig"/"falsch") stehen einmal statt pro Aussage wiederholt. Ctrl+B-Kürzel `V`.
- Drei neue, offene Blocktypen für Aufgaben ohne eine einzelne "richtige" Lösung — Arbeitsblatt und Lösung sehen bei ihnen identisch aus:
  - `:::mindmap` (Cluster-/Ideensammlung): Blockinhalt ist das zentrale Thema, gerendert als radiales SVG-Diagramm mit leeren Zweig-Feldern für eigene Assoziationen. Optionen `branches=` (Anzahl Zweige, 2-12, Standard `6`) und `shape=oval|rect|cloud` (Standard `oval`). Ctrl+B-Kürzel `U`.
  - `:::selfcheck` (Selbsteinschätzung/Reflexions-Skala): Blockinhalt ist eine Liste von Aussagen, jede mit einer wählbaren Symbol-Skala ohne markierte "richtige" Antwort. Optionen `scale=smiley|ampel|sterne|zahlen` (Standard `smiley`) und `steps=` (Skalenstufen, 2-7, Standard `3`). Ctrl+B-Kürzel `Ü`.
  - `:::writebox` (Rahmen-Freitext für offenes Schreiben): optionaler Schreibimpuls im Blockinhalt, gefolgt von einem dekorativ umrahmten Bereich mit Schreiblinien. Optionen `style=bubble|cloud|frame|letter` (Standard `frame`) und `lines=` (Anzahl Schreiblinien, 1-20, Standard `5`). Ctrl+B-Kürzel `Y`.
- `:::cloze` unterstützt jetzt auch `words=left`/`words=right` für die Wortbank-Position (bisher nur `above`/`below`); `:::wordsearch` bekommt eine neue `position=`-Option (links/rechts/oberhalb/unterhalb/`auto`, Standard weiterhin `below` -- bestehende Dokumente bleiben optisch unverändert) für die Wortliste relativ zum Rätselraster.
- Neuer Blocktyp `:::crossword` (Kreuzworträtsel): erzeugt aus einer Liste von Lösungswörtern mit Hinweisen (`words: [{word, clue}, ...]`) automatisch ein sich kreuzendes Rätselraster. Zellen sind nummeriert (Zahl + Pfeil zeigt die Richtung ▶/▼), die Hinweisliste ist nach "Waagerecht"/"Senkrecht" sortiert. Optionen: `maxw`/`maxh` (Rastergröße, Standard aus der Seitenbreite/-höhe abgeleitet), `prefill` (Anzahl zufällig vorausgefüllter Buchstaben im Arbeitsblatt-Modus), `position` (Hinweisliste links/rechts/unterhalb des Rasters, Standard `auto`), sowie optional `code` (ein frei wählbares Lösungscodewort, das sich aus Buchstaben der platzierten Wörter zusammensetzen lassen muss und im Lösungsmodus farbig hervorgehoben angezeigt wird) mit `code_row=true` (das Codewort läuft dann als eigene Zeile, alle anderen Wörter kreuzen es senkrecht). Lässt sich das Rätsel mit den gegebenen Wörtern/der Rastergröße nicht bauen oder das Codewort nicht bilden, erscheint eine klare Kompilierzeit-Warnung statt eines leeren Feldes. Ctrl+B-Kürzel `X`.
- Neue, blocktyp-übergreifende Layout-Option-Grundlage: eine Sekundärliste (z. B. eine Hinweisliste) kann links/rechts/oberhalb/unterhalb des Hauptinhalts positioniert werden (`position=left|right|above|below|auto`) -- aktuell genutzt vom neuen `crossword`-Block, weitere Blocktypen folgen.
- Neue Einstellung "Lebensdauer des Berechnungs-Caches" (Einstellungen → Allgemein, Standard "session"): steuert, ob eine kleine, versteckte Cache-Datei neben der Markdown-Datei (`.<dateiname>.blattwerk-cache`) beim Beenden von Blattwerk automatisch gelöscht wird oder zwischen Sitzungen erhalten bleibt. Die Datei enthält ausschließlich Performance-Zwischenergebnisse (z. B. künftig Kreuzworträtsel-Layouts) und kann jederzeit gefahrlos gelöscht werden.
- Autoren-Anleitungen fachlich vertieft: `docs/ANLEITUNG_ARBEITSBLATT_PRAESENTATION.md` erklärt jetzt explizit, dass `:::mc`-Antwortoptionen primär als Markdown-Checkbox-Liste im Blockinhalt stehen (`options=`/`correct=` sind nur ein Fallback), dass `:::table`-Zellen ausschließlich über `cells:`-YAML im Inhalt gefüllt werden (keine native Markdown-Tabelle), dass jeder Block sein eigenes `:::` braucht (keine Verschachtelung, gilt auch für `columns`/`nextcol`/`endcolumns`), und dass es in Präsentationen keinen Lösungs-Umschalter gibt (`mode=solution`/`:::solution` sind dort immer ausgeblendet). Jeder Blocktyp hat jetzt außerdem ein echtes, garantiert gültiges Beispiel (dasselbe, das Ctrl+B im Editor einfügt). `docs/ANLEITUNG_KURZENTWURF.md` stellt jetzt klar, dass `Hausaufgabe`/`Didaktische Reserve` trotz fehlendem `t=` die volle Segmentstruktur brauchen, dass `ant<` auch gute/neutrale antizipierte Antworten abdeckt (nicht nur Fehlvorstellungen), und dass Phasen mehrfach und in beliebiger Reihenfolge vorkommen dürfen.
- Zwei neue Stilempfehlungs-Dokumente (`docs/EMPFEHLUNGEN_STIL_ARBEITSBLATT_PRAESENTATION.md`, `docs/EMPFEHLUNGEN_STIL_KURZENTWURF.md`) sammeln reine Autor:innen-Präferenzen (z. B. Formulierungskonventionen, didaktische Platzierungsempfehlungen), getrennt von den normativen Hauptanleitungen.
...
- Autoren-Anleitung `docs/ANLEITUNG_ARBEITSBLATT_PRAESENTATION.md`: jede Blockoption hat jetzt eine eigene Tabellenzeile mit Kurzerklärung, akzeptierten Werten, Standardwert und ob der Wert vom Validator geprüft wird -- statt eines einzigen Prosa-Absatzes pro Blocktyp. Betrifft u. a. `cloze` (`gap`/`gap_length`/`words`/`words_multi`/`layout`), `matching`, `numberline`, `qrcode`, `table`, `wordsearch`, `mc`. Das Geometry-Beispiel nutzt jetzt Flow-Style-YAML (wie in den echten Beispieldokumenten) und ist als vollständiger, lauffähiger `:::geometry ...:::`-Block gezeigt; die Anleitung weist jetzt außerdem explizit darauf hin, dass `axis=true` ohne ein zugehöriges `origin` den Achsenmodus stillschweigend nicht aktiviert.
...
- Präsentations-Schnellstart-Vorlage (neue Präsentationsdokumente) nutzte `-+` (Frame-Aufbau, für dasselbe schrittweise aufgebaute Element gedacht) zwischen zwei inhaltlich verschiedenen Beispiel-Bausteinen -- das führt zu einer zunehmend überfüllten Folie statt eines echten Folienwechsels. Jetzt korrekt `--!` (erzwingt eine neue Folie).
...
- Menüleiste zeigte doppelte "Datei"- und "Ansicht"-Einträge: `BlattwerkAppBase.build_menu()` verwendete die deutschen Section-Keys `"datei"`/`"ansicht"` statt der von `BwBaseWindow` (bw-gui) erwarteten Keys `"file"`/`"view"`. Dadurch konnte `BwBaseWindow` die eigenen automatisch injizierten Einträge (Einstellungen, Theme-Umschalter) nicht mit Blattwerks echten Menüpunkten zusammenführen und hängte sie stattdessen als zusätzliche, unabhängige Menübuttons an. Keys auf `"file"`/`"view"` umgestellt (Labels unverändert).
...
- Long multiple-choice options no longer force global preview/export downscaling; MC labels now wrap inside available width instead of shrinking the whole worksheet rendering.
...
- Lernhilfen-Export als `PNG` und `PNG (ZIP)` rendert jetzt jede sichtbare Lernhilfe als eigenes Bild (nicht mehr seitenweise nach PDF-Seiten).
...
- Export dialog shortcut help text is now composed through the shared shortcut formatter, keeping key-label wording consistent with other shared GUI hover/shortcut surfaces.
...
- Lernhilfen preview window wiring now uses shared `bw_gui.runtime` aliases (`ui`, `widgets`) in `app/ui/blatt_ui_help_preview.py` instead of direct `tkinter`/`ttk` imports.
- Theme/menu/style wiring now uses shared `bw_gui.runtime` aliases (`ui`, `widgets`) in `app/ui/blatt_ui_style.py` instead of direct `tkinter`/`ttk` imports.
- Shared shell setup now uses `bw_gui.runtime.ui` in `bw_libs/app_shell.py` instead of direct `tkinter` imports.
- Export dialog wiring now uses shared `bw_gui.runtime` aliases (`ui`, `widgets`) in `app/ui/export_dialog.py` instead of direct `tkinter`/`ttk` imports.
- Shortcut registration now uses shared `bw_gui.runtime.ui` aliases in `app/ui/shortcut_manager.py` instead of direct `tkinter` imports.
- Settings and theme helpers now use shared `bw_gui.runtime` aliases (`ui`, `widgets`) in `app/ui/settings_dialog.py` and `app/ui/ui_theme.py` instead of direct `tkinter`/`ttk` imports.
- Persistence tab container/style wiring now uses shared `bw_gui.runtime.widgets` in `app/ui/blatt_ui_persistence.py` instead of direct `ttk` imports.
...
- Dialog-priority detection in shortcut runtime checks is now driven by the central popup policy stack (PopupPolicyRegistry-backed) instead of generic toplevel heuristics.
- Shortcut runtime debug is now opened as a dedicated popup window and no longer embedded in the preview area.
...
- Shortcut debug overlay now includes a compact table view with per-binding mode, key sequence, status, and disabled reason for faster diagnostics.
- New menu entry under `Datei -> Einstellungen direkt`: `Shortcut-Runtime-Debug` opens the diagnostics popup without needing a preview-area button.
