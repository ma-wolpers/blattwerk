...
- Neue Syntax `!!Operator!!` markiert einen Aufgaben-Operator (z. B. `!!Bestimme!! die Nullstellen.`) -- sieht optisch identisch zu `**fett**` aus, aber am Ende des Arbeitsblatts erscheint automatisch eine Legende mit den Definitionen aller tatsächlich verwendeten Operatoren (nicht in der Lösung, nicht im Kurzentwurf). Definitionen kommen aus einer zum Frontmatter-`Fach` passenden Datei in `data/operatoren/`; für den Start liegt ein Mathematik-Schema-Datensatz bei. Das Frontmatter-Feld `Stufe` (`5`-`13`, `E`, `Q1`, `Q2`, `Sek1`, `Sek2`) kann Operatoren zusätzlich auf bestimmte Klassenstufen einschränken und bekommt jetzt auch Editor-Autocomplete für seine Werte (wie schon `mode`/`document_type`).
